...
export default function BookingListing() {
  ...
  }, [bookings, itemsPerPage, currentPage])
  ...
  const paginatedData = Array.isArray(bookings) ? bookings.slice(startIndex, endIndex) : []
  const totalPages = Array.isArray(bookings) ? Math.ceil(bookings.length / itemsPerPage) : 1
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        ...
        <div className="text-sm text-gray-600 text-center sm:text-right">
          ...
          <span className="font-medium">{Math.min(endIndex, Array.isArray(bookings) ? bookings.length : 0)}</span> of{" "}
          <span className="font-medium">{Array.isArray(bookings) ? bookings.length : 0}</span> results
        </div>
      </div>
