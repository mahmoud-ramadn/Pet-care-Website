  ...
  return (
    <div className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 flex items-stretch gap-6 p-6">
      ...
      <div className="flex flex-col justify-between flex-grow">
        <div>
          <h3 className="text-2xl font-semibold mb-2 line-clamp-1">{shelterName}</h3>

          <div className="flex items-center gap-1 mb-3">
            {[...Array(5)].map((_, i) => (
              <Star key={i} size={18} className={i < rating ? "fill-yellow-400 text-yellow-400" : "text-gray-300"} />
            ))}
          ...
          {locations?.address && (
            <p className="text-gray-500 mb-4">
              <span className="font-medium">Opens 24 Hours</span> • {locations.address}
            </p>
          )}
        </div>
